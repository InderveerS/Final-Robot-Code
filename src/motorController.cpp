...
MotorController::MotorController(uint8_t mPin1, uint8_t mPin2, bool mInverted, 
    uint8_t mEncPin1, uint8_t mEncPin2, float kp, float ki, float kd, float dt, float alpha) : 
    motor(mPin1, mPin2, mInverted), pid(kp, ki, kd, dt, mOutMin, mOutMax, alpha), encoder(mEncPin1, mEncPin2) 
{
    this->mPin1 = mPin1;
    this->mPin2 = mPin2;
